...
export function TeamMembersWorkspace({
  ...
  const totalParticipations = useMemo(
    () => members.reduce((sum, member) => sum + member.participationCount, 0),
    [members],
  );

  const averageParticipations =
    members.length === 0 ? 0 : Math.round(totalParticipations / members.length);

  const teamStats = [
    { label: "Core team", value: members.length.toString() },
    { label: "Approved missions", value: approvedMissionsCount.toString() },
    { label: "Upcoming missions", value: upcomingMissionsCount.toString() },
  ...
  const onInviteSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const githubUsername = form.githubUsername.replace(/^@/, "").trim();

    if (!name || !role || !githubUsername) {
      return;
    }
              ...
              >
                ...
                <dl className="mt-5 grid gap-3 text-sm">
                  <div className="flex gap-2">
                    ...
                    <dd className="font-medium text-zinc-200">@{member.githubUsername}</dd>
                  </div>
                  <div className="flex gap-2">
        ...
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4">
          ...
          <div className="relative z-10 w-full max-w-md rounded-2xl border border-zinc-800 bg-zinc-900 p-6 shadow-2xl shadow-black/50">
            ...
            <p className="mt-1 text-sm text-zinc-400">
              Add a teammate to your MIGO core team workspace.
            </p>
